...
class CurrencyRepository:
    ...
    def _request_remote_data() -> ElementTree:
        print("CurrencyRepository#_request_remote_data -> Requesting remote data")
        res = get(constants.history_all_time_url)
        print("CurrencyRepository#_request_remote_data -> Remote data requested")
        return elementTree.fromstring(res.content.decode("utf-8"))

    @staticmethod
    def _parse_xml_content(content: ElementTree) -> [Currency]:
        print("CurrencyRepository#_parse_xml_content -> Beginning xml parsing")
        currency_list = []
        ...
        print("CurrencyRepository#_parse_xml_content -> Xml parsing completed")
    ...
    def __filter_local_list(currency_data: [Currency], start_at: [str, None], end_at: [str, None],
        ...
        print("CurrencyRepository#__filter_local_list -> Local data filter complete")
    ...
    def _verify_and_update_cache(self) -> tuple[bool, [list, None]]:
        print("CurrencyRepository#_verify_and_update_cache -> Checking cache data")
        if not self._cache_repository.get_valid_cache().is_valid:
            ...
            print("CurrencyRepository#_verify_and_update_cache -> Cache is valid")
            return True, currency_data
        print("CurrencyRepository#_verify_and_update_cache -> Cache is invalid")
        return False, None

    @staticmethod
    def normalize_by_base(base_currency_code: str, currency_data: [Currency]) -> Generator[Currency, None, None]:
        print(f"CurrencyRepository#_verify_and_update_cache -> Normalizing currency data with params: "
        ...
        for currency in currency_data:
            ...
            yield currency
        print("CurrencyRepository#_verify_and_update_cache -> Currency data normalized")
    ...
    def get_all(self, base_currency_code: str, start_at: [str, None], end_at: [str, None], symbols: [[str], None]) -> \
        ...
        if cache[0]:
            print("CurrencyRepository#get_all -> Retrieving data remotely with params: ")
            print(f"base_currency_code = {base_currency_code}, "
                     f"start_at = {start_at}, end_at = {end_at}, symbols = {symbols}")
            return self.__filter_local_list(list(self.normalize_by_base(base_currency_code, cache[1])), start_at,
                                            end_at, symbols)
        else:
            print("CurrencyRepository#get_all -> Retrieving data locally")
            print(f"base_currency_code = {base_currency_code}, "
                     f"start_at = {start_at}, end_at = {end_at}, symbols = {symbols}")
            filters = {}
            if start_at:
    ...
    def get_latest(self, symbol: str) -> Currency:
        cache = self._verify_and_update_cache()
        if cache[0]:
            print(f"CurrencyRepository#latest -> Retrieving data remotely with params: symbol = {symbol}")
            return self.__filter_local_list(cache[1],
                                            (datetime.datetime.now() - datetime.timedelta(days=1)).strftime("%Y-%M-%d"),
                                            datetime.datetime.now().strftime("%Y-%M-%d"), [symbol])[0]
        else:
            print(f"CurrencyRepository#latest -> Retrieving data locally with params: symbol = {symbol}")
            return self._currency_dao.select_one({"symbols": symbol}, ("historical_data", pymongo.DESCENDING))

    def insert(self, *currency):
        print(f"CurrencyRepository#insert -> Inserting objects: {[*currency]}")
        self._currency_dao.insert(*currency)

    def delete(self, *currency):
        print(f"CurrencyRepository#delete -> Deleting objects: {[*currency]}")
        self._currency_dao.delete(*currency)

    def select_one(self, ftr: dict) -> Currency:
        print(f"CurrencyRepository#select_one -> Retrieving one object with filter ")
        print(ftr)
